---
description: 獲得、エンゲージメント、イベント、コンバージョン、リテンションのキャンペーンをまたいで、ユースケースごとに整理された、AdobeのWorkfront キャンペーンの実際のプロンプトパターンを確認できます。
title: ユースケース
source-git-commit: 5de0afec89309ab62bd50e0c2cb1d0e96890c57f
workflow-type: tm+mt
source-wordcount: '502'
ht-degree: 1%

---

# ユースケース {#use-cases}

以下に、ライフサイクルマーケターやグロースマーケターがAdobe Coworker Campaignsで使用している実際のプロンプトパターンを示します。 それぞれがコピーされるように構築され、独自のオーディエンスやブランドの詳細と入れ替わり、会話で洗練されています。

## 使用方法

目標に合ったユースケースを選択し、サンプルプロンプトをコピーしてCoworker Campaignsに貼り付け、プレースホルダーを詳細に置き換え、チャットを通じて出力を調整します。 ステップバイステップの詳細については、キャンペーンの構築と生成に進みます。

## 顧客獲得とオンボーディング

新規顧客、メンバー、アカウントを歓迎し、長期的なエンゲージメントに活用できます。

### 新規顧客オンボーディングシリーズ、B2B

最適な用途：エンタープライズアカウントのアクティベーション、新しいSaaS サインアップ、販売後のハンドオフ

`"Create a guided onboarding email series for new enterprise accounts. Include a welcome email, a setup walkthrough, and a check-in from a customer success contact. Use the CSV I will upload and personalize by first name and company name."`

### B2C、新規会員オンボーディング

最適な用途：メンバーシッププログラム、アプリのサインアップ、ロイヤルティプログラム

`"Create a marketing campaign to onboard new members with a welcome email series that introduces key features, provides getting-started guides, and offers personalized recommendations based on their interests."`

### ブランド導入順序，B2C

最適な用途：初回購入者、ニュースレターの新規登録者、登録後のフォローアップ

`"Create a single-touch follow-up email introducing our brand, what we sell, and how to place a first order. Use a CSV list, and personalize by first name and company name."`

### 新しいファンオンボーディングジャーニー、B2C （スポーツおよびエンターテイメント）

最適な用途：初めてのチケット購入者、新しいシーズンのチケット所有者、購入後のファンの活性化

`"Create a 4-email post-purchase journey for new fans that introduces the season, loyalty benefits, and next events."`

## エンゲージメントとナーチャリング

顧客に最新の情報を提供し、必要な瞬間に必要な情報を提供して、つながりを維持できます。

### 月刊ニュースレター、B2B

最適な用途：顧客維持、製品認知度、ソートリーダーシップ

`"Create a monthly newsletter for our customers. Use the CSV I will upload. Include product updates, industry insights, and a resource, with simple CTAs."`

### 月刊ニュースレター、B2C

最適な用途：小売、ライフスタイル、e コマースの企業

`"Create a monthly newsletter for our customers. Highlight new products, inspiration ideas, and a featured collection with CTAs."`

### 商品ヒントシリーズ、B2B

最適な用途：製品エデュケーション、機能の導入、価値実現までの時間の短縮

`"Generate an educational 2-touch email nurture that helps users get more value from this product with simple tips. Use a CSV uploaded list and personalize by first name, product used, and signup date."`

### 季節カタログの発売、B2C

最適な用途：季節限定のプッシュ、コレクションのドロップ、モーメントの更新。

`"Build a 2-touch email campaign, 3 days apart, announcing our new seasonal catalog. Highlight what’s new and relevant for buyers in my CSV list. Personalize content to each person’s first name, product interest category, and region."`

## イベントとモーメント主導

ウェビナー、リリース、ライブモーメントの登録、出席、フォローアップを促進します。

### ウェビナーの招待状キャンペーン、B2B

最適な用途：リードジェネレーション、製品エデュケーション、ソートリーダーシップ

`"Create a webinar invite about [TOPIC]. I need one invite email and one reminder email with a clear registration CTA."`

### ウェビナーのフォローアップキャンペーン、B2B

最適な用途：イベント後のナーチャリング、MQL コンバージョン、コンテンツ配信

`"Create a webinar follow-up campaign for customers who attended a webinar about [TOPIC]. Include a strong CTA."`

### 製品発表、B2BおよびB2C

最適な用途：ローンチ、早期アクセス、新機能の発表

`"Create a product announcement campaign for our [PRODUCT NAME] launch. Build an early-access invitation and a launch-day email."`

### テーマ別イベント/チケットプッシュ、B2C （スポーツ&amp;エンターテインメント）

最適な用途：シングルゲームチケット販売、テーマ別の夜、特別なイベント。

`"Create a 2-touch email campaign to drive ticket sales for a themed home game event."`

### B2C （スポーツおよびエンターテイメント）のエンジンを使用した最適化により、目的地に到着する前に把握しましょう

最適なアプローチ：ゲームデイおよびイベントデイのファンへの対応

`"Create a 3-touch campaign that prepares fans for game day and encourages attendance."`

## コンバージョンとアップセル

オーディエンスを、興味から購入へ、または階層から次の階層に移動できます。

### 無料トライアルへのコンバージョン、B2B

最適なソリューション：SaaS トライアルユーザー、フリーミアムアップグレード、トライアル後のアクティベーション

`"Generate a campaign to target users whose trial has ended but are still active in the app, offering them a special incentive to convert to a paid plan."`

### 再発注ナッジ、B2C

最適な用途：消耗品、補充カテゴリー、リピート購入サイクル

`"Give me a 1-step reorder reminder email that nudges purchasers to restock based on typical buying cycles. Use my CSV list with each purchaser's first name, last order date, and product last purchased to tailor the content."`

### プレミアムエクスペリエンスのアップセル、B2C （スポーツおよびエンターテイメント）

最適な用途：標準チケット所有者をプレミアム階層にアップグレードします。

`"Create a personalized 3-email campaign to move fans from standard tickets into premium tiers."`

### B2C （スポーツ&amp;エンターテインメント）のシーズンメンバーシップオンボーディング

最適な用途：シーズンチケットのアクティベーション、プレミアムメンバーシップ、セグメントに合わせてパーソナライズされたウェルカムメッセージ

`"Create a 4-email welcome series for new season plan holders with segment-personalized benefits."`

## リテンションとリエンゲージメント

ドリフトしたオーディエンスを取り戻し、休眠セグメントを再活性化しましょう。

### 顧客の再訪問を促すメールシリーズ、B2C

最適なアプローチ：顧客離れ、休眠中のバイヤー、季節ごとのリエンゲージメント

`"Create a win-back email series for customers who bought last year but haven't come back. Use the CSV I will upload. Include 2-3 emails that feel seasonal and remind them to shop again."`

### 解約したファンとのリエンゲージメント、B2C （スポーツおよびエンターテイメント）

最適な用途：非アクティブな購読者、休眠シーズンのチケット所有者、シングルゲームのコンバージョン

`"Create a 3-email win-back campaign to convert lapsed fans into single-game ticket buyers."`

### 一般的な「ウィンバック」施策、B2BおよびB2C

最適な用途：静かになったオーディエンス

`"Create a win-back campaign targeting inactive customers who haven't engaged recently. Highlight what's new and offer special incentives to re-engage."`

## 構築準備が整ったら？

上記のユースケースを選択し、完全な手順を説明する電子メールキャンペーン ](./)を[作成して、試してみてください。
