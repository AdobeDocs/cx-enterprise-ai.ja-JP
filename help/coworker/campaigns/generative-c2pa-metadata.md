...
## メールキャンペーンでのContent Credentials {#content-credentials-email}

メールキャンペーンで送信された画像はContent Credentialsを維持するので、受信者は配信されたメールから直接画像の出所と信頼性を確認できます。
...
Coworker Campaignsは、サポートされている画像アセットに関連付けられているContent Credentialsを保持します。 Coworker Campaignsに読み込まれた画像にContent Credentialsが含まれている場合、生成されたキャンペーンコンテンツとアウトバウンドメールエクスペリエンスでアセットを使用する際に、これらの資格情報が保持されます。
